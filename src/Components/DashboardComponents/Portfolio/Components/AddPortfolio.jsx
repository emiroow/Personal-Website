...
export default function AddPortfolio({ setNewServerState }) {
    ...
    const handleAddPortfolio = async () => {
        if (link.current.value && imageUrl.current.value && description.current.value && title.current.value && editeState.length !== 0) {
            const response = await dispatch(fetchSetAdminPortfolio({
                portfolioId: 0,
        ...
        <div className='w-[95%] mt-5 lg:w-[40%] bg-BackColorWhiter text-white p-7 border-2 dark:border-DarkPurple rounded-xl'>
            <div className='flex justify-between  max-lg:flex-col'>
                <div className='flex flex-col lg:w-[49%] mb-4'>
                    ...
                    <input ref={title} type="text" className={` text-center shadow-[0px_0px_10px_0px_rgba(0,0,0,0.35)] text-black h-10 mt-1 rounded-md mb-2 outline-none  dark:shadow-[0px_0px_10px_0px_rgba(0,0,0,0.25)]`} />
                </div>
                <div className='flex flex-col lg:w-[49%] mb-4'>
                    <label>{t("link")} :</label>
                    <input ref={link} className=' text-center shadow-[0px_0px_10px_0px_rgba(0,0,0,0.35)] text-black h-10 mt-1 rounded-md mb-2 outline-none  dark:shadow-[0px_0px_10px_0px_rgba(0,0,0,0.25)]' />
                </div>
            </div>
            <div className='flex justify-between max-lg:flex-col'>
                <div className='flex flex-col lg:w-[49%] mb-4'>
                    <label>{t("ImageUrl")} :</label>
                    <input ref={imageUrl} type="text" className={` text-center shadow-[0px_0px_10px_0px_rgba(0,0,0,0.35)] text-black h-10 mt-1 rounded-md mb-2 outline-none  dark:shadow-[0px_0px_10px_0px_rgba(0,0,0,0.25)]`} />
                </div>
            </div>
            <span>{t("descriptionLable")} :</span>
            <textarea ref={description} className='w-full rounded-xl mt-2 p-3 text-black' rows="5"></textarea>
                                    ...
                                    return (
                                        <tr key={item.portfolioId} className="border-b border-neutral-500 bg-neutral-700">
                                            ...
                                            <td className="whitespace-nowrap px-6 py-4 flex text-center justify-center ">
                                                <input type="checkbox" onChange={() => handleChangeCategory(item.catagoryId)} className='text-2xl cursor-pointer accent-pink-500' />
                                            </td>
                                        </tr>
                                    )
                                })
            ...
            <div className='w-full mt-3 flex'>
                <div className='w-full mt-1 flex p-1'>
                    <button onClick={handleAddPortfolio} className='text-sm ml-5 bg-green-500 font-IranLight rounded-lg p-2'>{t("SaveBtn")}</button>
