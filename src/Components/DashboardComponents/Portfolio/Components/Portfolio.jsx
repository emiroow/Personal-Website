...
export default function Portfolio() {
    ...
    const Portfolios = useSelector((state) => state.portfolio.portfolios.filter((item) => item.lang === TabState))
    ...
    return (
        <div className='w-full mt-16'>
            ...
            <div className='mt-5 w-full flex flex-row flex-wrap justify-around'>
                {
                    newServerState ? <AddPortfolio setNewServerState={setNewServerState} /> : null
                }
                {
                    Portfolios.map((item) => {
                        return (
                            <PortfolioShow data={item} />
                        )
                    })
                }
            </div>
