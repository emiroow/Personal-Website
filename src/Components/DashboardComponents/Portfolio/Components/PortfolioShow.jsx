...
export default function PortfolioShow({ data }) {
    ...
    useEffect(() => {
        if (modalState.Access && modalState.id) {
            const deletDatefromServer = async () => {
                try {
                    const response = await dispatch(fetchDeleteAdminPortfolio(modalState?.id))
    ...
    const HandleSaveEditing = async () => {
        if (title.current.value && imageUrl.current.value && link.current.value && editeState.length !== 0) {
            const response = await dispatch(fetchEditAdminPortfolio({
    ...
    return (
        <div className='w-[95%] mt-5 lg:w-[40%] bg-BackColorWhiter text-white p-7 border-2 dark:border-DarkPurple rounded-xl'>
            ...
            <div>
                <div className='flex justify-between  max-lg:flex-col'>
                    <div className='flex flex-col lg:w-[49%] mb-4'>
                        ...
                        <input ref={title} disabled={editeing ? false : true} defaultValue={data?.title} type="text" className={` text-center shadow-[0px_0px_10px_0px_rgba(0,0,0,0.35)] text-black h-10 mt-1 rounded-md mb-2 outline-none  dark:shadow-[0px_0px_10px_0px_rgba(0,0,0,0.25)]`} />
                    </div>
                    <div className='flex flex-col lg:w-[49%] mb-4'>
                        <label>{t("link")} :</label>
                        <input ref={link} disabled={editeing ? false : true} defaultValue={data?.link} className=' text-center shadow-[0px_0px_10px_0px_rgba(0,0,0,0.35)] text-black h-10 mt-1 rounded-md mb-2 outline-none  dark:shadow-[0px_0px_10px_0px_rgba(0,0,0,0.25)]' />
                    </div>
                </div>
                <div className='flex justify-between max-lg:flex-col'>
                    <div className='flex flex-col lg:w-[49%] mb-4'>
                        <label>{t("ImageUrl")} :</label>
                        <input ref={imageUrl} disabled={editeing ? false : true} defaultValue={data?.imageUrl} type="text" className={` text-center shadow-[0px_0px_10px_0px_rgba(0,0,0,0.35)] text-black h-10 mt-1 rounded-md mb-2 outline-none  dark:shadow-[0px_0px_10px_0px_rgba(0,0,0,0.25)]`} />
                    </div>
                </div>
                <span>{t("descriptionLable")} :</span>
                <textarea ref={description} disabled={editeing ? false : true} defaultValue={data?.description} className='w-full rounded-xl mt-2 p-3 text-black' rows="5"></textarea>
                            ...
                            <tbody>
                                {
                                    Categories.map((item, index) => {
                                        return (
                                            <tr key={item.portfolioId} className="border-b border-neutral-500 bg-neutral-700">
                                                ...
                                                <td className="whitespace-nowrap px-6 py-4 flex text-center justify-center ">
                                                    <input type="checkbox" onChange={() => handleChangeCategory(item.catagoryId)} defaultChecked={HandleSelectedCategory(item.catagoryId)} className='text-2xl cursor-pointer accent-pink-500' disabled={editeing ? false : true} />
                                                </td>
                                            </tr>
                                        )
                                    })
            ...
            <div className='flex flex-row mt-5'>
                {HandleEditing()}
                <button onClick={() => handleDeletDate(data?.portfolioId)} className='text-sm mx-3 w-max bg-red-500 font-IranLight rounded-lg p-2'>{t("RemoveBtn")}</button>
            </div>
        </div>
    )
}
