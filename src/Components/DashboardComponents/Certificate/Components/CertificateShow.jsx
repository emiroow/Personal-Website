...
import { fetchDeleteAdminCertificate, fetchEditAdminCertificate } from '../../../../Reducers/DashboardSlices/CertificateSlice';
export default function CertificateShow({ data, TabState }) {
    ...
    const HandleSaveEditing = async () => {
        ...
        if (subjectVal &&
            ...
            messageVal &&
            fromImgUrlVal) {
            const response = await dispatch(fetchEditAdminCertificate({
                id: data.id,
                title: subjectVal,
    ...
    useEffect(() => {
        const setServer = async () => {
            if (modalState.Access) {
                const response = await dispatch(fetchDeleteAdminCertificate(modalState?.id))
                if (response.payload.status === 200) {
                ...
                <div className=" border-y-2 pb-8 dark:border-DarkPurple w-full  p-4 rounded-xl  mr-auto ">
                    <div className='flex justify-between  max-lg:flex-col'>
                        <div className='flex flex-col lg:w-[49%] mb-4'>
                            ...
                            <input defaultValue={data?.title} ref={subject} disabled={editeing ? false : true} type="text" className={` text-center shadow-[0px_0px_10px_0px_rgba(0,0,0,0.35)] text-black h-10 mt-1 rounded-md mb-2 outline-none  dark:shadow-[0px_0px_10px_0px_rgba(0,0,0,0.25)]`} />
                        </div>
                        <div className='flex flex-col lg:w-[49%] mb-4'>
                    ...
                    <div className='flex flex-col mb-4'>
                        ...
                        <input defaultValue={data?.link} ref={fromImgUrl} disabled={editeing ? false : true} type="text" className={` text-center shadow-[0px_0px_10px_0px_rgba(0,0,0,0.35)] text-black h-10 mt-1 rounded-md mb-2 outline-none  dark:shadow-[0px_0px_10px_0px_rgba(0,0,0,0.25)]`} />
                    </div>
                    <div className='flex flex-col  mb-5'>
                        <label>{t("descriptionLable")} :</label>
                        <textarea defaultValue={data?.description} ref={message} disabled={editeing ? false : true} className='outline-none p-3 shadow-[0px_0px_10px_0px_rgba(0,0,0,0.35)] text-black rounded-xl mt-2' id="" rows="5"></textarea>
                    </div>
