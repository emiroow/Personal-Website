...
import { fetchDeleteAdminCircleSkill } from '../../../Reducers/DashboardSlices/SkillsSlice';

export default function CircelProgresContentsEn() {
    const loader = useSelector((state) => state.socials.status === "pending" ? true : false)
    const CircleSkills = useSelector((state) => state.skills.CircleSkills.filter((item) => item.lang === 0))
    const dispatch = useDispatch()
    const { t } = useTranslation()

    const handleDelete = async (id) => {
        const response = await dispatch(fetchDeleteAdminCircleSkill(id))
