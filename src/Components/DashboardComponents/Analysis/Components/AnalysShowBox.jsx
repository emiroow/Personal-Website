...
} from "../../../../Reducers/DashboardSlices/AnalysisSlice";
import Modal from "../../Modal";
export default function AnalysShowBox({
  value,
  title,
  id,
  SetupdateStates,
  ...
  const handleGetAllChangs = () => {
    SetAnalysis({
      ...
      [valueRef.current.name]: valueRef.current.value,
      id: parseInt(titleRef.current.id),
    });
  };
  ...
  const HandleEditing = () => {
    if (!editeing) {
      ...
      return (
        <>
          <button
            onClick={handleGetAllChangs}
            className="text-sm bg-green-500 font-IranLight rounded-lg p-2"
          >
            {t("SaveBtn")}
          </button>
          <button
            ...
            className="text-sm bg-amber-500 font-IranLight rounded-lg p-2"
          >
            {t("CancelLocation")}
          </button>
        </>
  ...
  return (
    <div className=" w-[95%] lg:w-[23%] border-2 dark:border-white border-LightMaincolor  mx-3 my-3 rounded-lg p-2 text-lg ">
      ...
      <input
        ...
        type="title"
        className="w-full border-2 my-1 rounded-lg outline-none h-10 text-MainColorDark border-emerald-400 text-center mb-3"
        defaultValue={title}
      />
      <label className="dark:text-white text-LightMaincolor">
        {t("ValueLabel")} :
      ...
      <input
        ...
        disabled={editeing ? false : true}
        type="value"
        className="w-full border-2 my-1 rounded-lg outline-none h-10 text-MainColorDark text-center border-emerald-400"
        defaultValue={value}
      />
      <div className="w-full mt-1 flex justify-around p-1">
        ...
        <button
          ...
          className="text-sm bg-red-500 font-IranLight rounded-lg p-2"
        >
          {t("RemoveBtn")}
        </button>
      </div>
