...
} from "../../../../Reducers/DashboardSlices/EducationsSlice";
  ...
  useEffect(() => {
    if (modalState.Access && modalState.id) {
      const deletDatefromServer = async () => {
        try {
          const response = await dispatch(
            fetchDeleteAdminEducation(modalState?.id)
          );
          if (response.payload.status === 200) {
      ...
      <div className="flex md:contents">
        <div className=" border-y-2 pb-8 dark:border-DarkPurple w-full  p-4 rounded-xl  mr-auto ">
            ...
            <DatePicker
              ...
              value={
                i18n.language === "fa"
                  ? data?.dateTime &&
                    moment
                      .from(data?.dateTime, "en", "YYYY-MM-DDTHH:mm")
                      .locale("fa")
          ...
          <div className="flex flex-row ">
            {HandleEditing()}
            <button
              onClick={() => handleDeletDate(data?.id)}
