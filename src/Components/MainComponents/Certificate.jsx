...
export default function Certificate() {
    ...
    return (
        <div id='Portfolio' className=' my-5 md:my-16 duration-700 justify-center flex flex-col items-center transition-all'>
            <div className='w-[98%] md:w-[95%] 2xl:w-[95%] max-md:m-auto flex justify-between items-center'>
                <h1 className='text-shadow-dark font-IranBold text-xl md:text-2xl 2xl:text-3xl md:mx-5'>{t("Certificate")}</h1>
            </div>
            <Fancybox options={{ infinite: true }}>
                <div className='w-full flex-wrap justify-center md:justify-center px-5 mt-5 flex'>
                    {
                        getData?.map((item) => {
                            return <div key={item.id} className=" shadow-[0px_0px_10px_0px_rgba(0,0,0,0.40)] h-max max-lg:mb-4 rounded-lg dark:border-DarkPurple border-LightMaincolor border-[2.5px] md:mx-4 2xl:mx-5 ">
