...
export default function History() {
  ...
  const getHistoriesData = useSelector(
    (store) => store.client.clientState.histories
  );
  const getEducationsData = useSelector(
    (store) => store.client.clientState.educations
  );
  const getData = useSelector((store) => store.client.clientState);
  const getSetting = useSelector((store) => store.setting.setting);
  return (
    <>
      {getSetting?.educations || getSetting?.histories ? (
        <div className="w-full " id="date">
