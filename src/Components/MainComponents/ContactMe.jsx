...
export default function ContactMe() {
  ...
  const handleSendContact = async (values, resetForm) => {
    const response = await dispatch(fetchContactMessage(values));
  ...
  const schema = Yup.object().shape({
    ...
    message: Yup.string()
      .min(10, t("minCharacter"))
      .required(t("ContactMessage")),
  });
  ...
  return (
    <div id="contact" className="my-5 md:my-16">
      ...
      <div className=" m-auto w-[98%] md:w-[95%] 2xl:w-[93%]">
        <h1 className="text-shadow-dark mb-5 max-md:mt-10 font-IranBold text-xl md:text-2xl  2xl:text-3xl">
          {t("Cuntact")}
        </h1>
      </div>
        ...
        <div className="w-full lg:w-[69.5%] dark:bg-BackColorWhiter rounded-md p-5 border-y-2 dark:border-DarkPurple border-LightYellow bg-LightMaincolor h-max">
          <Formik
            ...
            onSubmit={(values, { resetForm }) => {
              handleSendContact(values, resetForm);
            }}
            className="w-full"
          >
            {({
              ...
              <form onSubmit={handleSubmit}>
                <div className="w-full">
                  <label htmlFor="" className="font-IranBold text-sm">
                    {t("Fullname")}
                  </label>
                  <input
                    type="text"
                    ...
                    className="w-full p-2 font-IranLight mt-1 rounded-md dark:bg-DarkPurple/40 bg-[#293462]/50 outline-none px-2 "
                  />
                ...
                <div className="w-full mt-3">
                  <input
                    className="flex items-center cursor-pointer justify-center dark:text-white text-sm  2xl:text-md text-LightBackcolor dark:bg-DarkPurple bg-LightYellowDark rounded-md text-center p-2 font-IranBold"
                    type="submit"
                    value={t("Sendmessage")}
                  />
                </div>
          ...
          <div className="w-full dark:bg-BackColorWhiter rounded-md p-5 border-y-2 dark:border-DarkPurple border-LightYellow bg-LightMaincolor ">
            ...
            <div className="w-full text-sm md:text-md 2xl:text-lg ">
              <div className="flex justify-between mt-5">
                <span className="font-IranBold text-[14px] md:text-[18px] lg:text-[14px]">
                  {t("HeadInfoCountry")} :
                </span>
                <span className="font-IranLight text-[13px] md:text-[18px] xl:text-[11px] 2xl:text-[15px]">
                  {getAboutData?.country}
                </span>
              </div>
              <div className="flex justify-between mt-5">
                <span className="font-IranBold text-[14px] md:text-[18px] lg:text-[14px]">
          ...
          <div className="w-full dark:bg-BackColorWhiter rounded-md p-5 border-y-2 dark:border-DarkPurple border-LightYellow bg-LightMaincolor  mt-[6px]">
            ...
            <div className="w-full text-sm md:text-md 2xl:text-lg ">
              <div className="flex justify-between mt-5">
                ...
                <span className="font-IranLight text-[13px] xl:text-[11px] 2xl:text-[15px] break-all">
                  {getAboutData?.email}
                </span>
              </div>
              <div className="flex justify-between mt-5">
                <span className="font-IranBold text-[14px] md:text-[18px] lg:text-[14px]">
                  {t("ContactShowPhoneNumber")} :
                </span>
                <span className="font-IranLight text-[13px] md:text-[18px] xl:text-[11px] 2xl:text-[15px]">
                  {getAboutData?.phoneNumber}
                </span>
              </div>
            </div>
          </div>
