  ...
  return (
    <div className="lg:w-[25%] lg:block hidden relative h-[96vh] ">
      <div className="w-[24.4%] h-[97vh] fixed  dark:bg-BackColor bg-LightMaincolor shadow-[0px_0px_10px_0px_rgba(0,0,0,0.25)] rounded-md border-b-2 dark:border-DarkPurple border-LightYellow">
        ...
        <div
          className={`${
            getSetting?.social && getData?.socials ? "h-[56vh]" : "h-[62vh]"
          } pb-5 overflow-auto RemoveScroll cursor-pointer`}
        >
          {getSetting?.info ? <Info /> : null}
          {getSetting?.circleSkills && getData?.circleSkills !== 0 ? (
            <CircleProgressBarss />
          ) : null}
          {getSetting?.skills && getData?.skills !== 0 ? (
            <SkilsLineProgress />
          ) : null}
          {getSetting?.otherSkills &&
          getData?.skills?.filter((item) => item.progressBar === false)
            .length !== 0 ? (
            <AdditionalSkills />
          ) : null}
          {getSetting?.downloadCv && getData?.about?.cvUrl ? (
            <DownloadCv />
          ) : null}
        </div>
        {getSetting?.social && getData?.socials !== 0 ? <Socials /> : null}
